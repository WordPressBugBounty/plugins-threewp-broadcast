	...
	**/
	public function update_attachment_ids( $bcd, $content )
	{
		foreach( $bcd->copied_attachments() as $a )
		{
			...
			if ( $count > 0 )
				$this->debug( 'Modified Gutenberg images: %s times', $count );
			...
			foreach( $guids as $old_guid => $new_guid )
			{
				...
				foreach( $matches[ 0 ] as $index => $old_match )
				{
					...
					if ( $count > 0 )
					{
						$this->debug( 'a: Modified wp-att: %s times', $count );
						$content = str_replace( $old_match, $new_match, $content );
						$old_match = $new_match;
					}
				}
				...
				foreach( $matches[ 0 ] as $index => $old_match )
				{
					...
					if ( $count > 0 )
					{
						$this->debug( 'img: Modified attachment ID: %s times', $count );
						$content = str_replace( $old_match, $new_match, $content );
						$old_match = $new_match;
					}
					...
					if ( $count > 0 )
					{
						$this->debug( 'img: Modified wp-image ID class: %s times', $count );
						$content = str_replace( $old_match, $new_match, $content );
						$old_match = $new_match;
					}
				}
